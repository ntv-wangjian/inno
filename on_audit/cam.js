...
Cam.prototype.connect = function(callback) {
	...
	this.getSystemDateAndTime(function(err, date, xml) {
		if (err) {
			/*
			 * Modified by --WJ 原来没有使用return ，导致出错后继续执行后续操作，出现连接两次、触发超时两次 的现象。
			 * 但是，如果不是严重错误，譬如仅仅是没有时间信息的话，也有可能后续流程可以继续执行
			 */
...
Cam.prototype._request = function(options, callback) {
	...
	var reqOptions = options.url || {
			...
			, port: this.port
			,timeout:this.timeout     //Added BY --WJ  把超时设置真正传进来，原来是有BUG的。
	...
	req.setTimeout(this.timeout, function() {
		...
		/*
		 * Added by WJ  这句话很重要，不然异步操作会一直在等待什么。
		 */
		this.abort();
	});
